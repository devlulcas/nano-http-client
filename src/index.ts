...
export class HttpClient {
  ...
  constructor(options: HttpClientOptions) {
    const { baseUrl, customErrorHandler, customFetch, ...requestInit } =
      options;
   ...
   */
  private getSearchParamsFromRequestInit(
   ...
   */
  private getBodyFromRequestInit(
    requestInit: CustomRequestInit
  ): BodyInit | undefined {
  ...
  public async request<T>(
    ...
    if (response.ok) {
      const shouldParseAsJson = response.headers
        ?.get('content-type')
        ?.includes('application/json');

      const data = shouldParseAsJson
        ? await response.json()
        : await response.text();
...
async function transformThrowIntoResponse(
  fn: () => Promise<Response>
): Promise<Response> {
  try {
    return fn();
